...
import { NoHomeRunsToday } from "./no-home-runs-today"
...
interface RecentHomeRunsWidgetProps {
  playerIds: string[]
}

export function RecentHomeRunsWidget({ playerIds }: RecentHomeRunsWidgetProps) {
  ...
  const fetchHomeRuns = async () => {
    ...
    try {
      const response = await fetch(`/api/recent-home-runs?playerIds=${playerIds.join(",")}`)
  ...
  const debugHomeRuns = async () => {
    setIsDebugging(true)
    try {
      const response = await fetch(`/api/debug-home-runs?playerIds=${playerIds.join(",")}`)
      const data = await response.json()
      setDebugInfo(data)
    } catch (error: any) {
  ...
  // Replace the formatDate function with the improved version:
  ...
  if (error) {
    return <NoHomeRunsToday isDebugging={isDebugging} debugHomeRuns={debugHomeRuns} debugInfo={debugInfo} />
  }
