...
export default function UpdateAllStatsPage() {
  ...
  const triggerUpdate = async () => {
    ...
    try {
      // Use the standalone endpoint directly for more reliability
  ...
  return (
    <div className="container py-12">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-4xl font-bold mb-2">Update All Team Stats</h1>
          <p className="text-muted-foreground">Manually update current season home run stats for all teams</p>
        </div>
        ...
        <CardContent>
          ...
          {result && (
            <Alert variant={result.success ? "default" : "destructive"} className="mb-6">
              ...
              <AlertDescription>
                {result.error || result.message || "Stats updated successfully"}

                {result.teams && result.teams.length > 0 && (
                  <div className="mt-2">
                    <p>Successfully updated {result.teams.length} teams</p>
                    {result.dataSource && <p className="text-sm">Data Source: {result.dataSource}</p>}
                    <div className="mt-2 max-h-40 overflow-y-auto text-xs">
                      <table className="w-full">
                        ...
                        <tbody>
                          {result.teams.map((team: any, index: number) => (
